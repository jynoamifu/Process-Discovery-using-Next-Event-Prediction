# -*- coding: utf-8 -*-
"""BPIC_Preprocessing.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1JbWHeqTIJ-Ked-gHf--yHLfQzjTIJPJl
"""
...
#List of integers mapped to integers --> Use this word index to pad validation and test set.
word_index = tokenizer.word_index
print(word_index)
print('Found %s unique tokens.' % len(word_index))

# Play with the pad_sequence parameters
# Change prefix length: adding a lot of 0 or losing information 
# If prefix too long: cut off at the beginning "pre"
# If prefix too short: 
X_train = pad_sequences(X_train, maxlen = prefix_len, padding = 'pre', truncating = 'pre', value=0)
print(X_train.shape)
print(X_train)
...
#tokenizer = Tokenizer(filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')
#tokenizer.fit_on_texts(event_log_val['X_val'])
...
X_val = pad_sequences(X_val, maxlen = prefix_len, padding = 'pre', truncating = 'pre', value = 0)
print(X_val.shape)
print(X_val)
...
# Convert activities/events to tokens.
#tokenizer = Tokenizer(filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')
#tokenizer.fit_on_texts(event_log_test['X_test'])
...
# Use padding to ensure all sequences are of same length.
X_test = pad_sequences(X_test, maxlen = prefix_len, padding = 'pre', truncating = 'pre', value = 0)
print(X_test.shape)
print(X_test)
...
# Opmerking Jari: waarom batch size equal to vocab size? Neem random: machten van 2: 64, 128, ...
...
#model = keras.models.load_model('LSTM_BPIC2012.h5')
...
predictions['last_activity'] = predictions.iloc[:, :prefix_len].ffill(axis=1).iloc[:, -1].fillna('NULL')
